    ...
    ReentrancyGuard  
{
        ...
        returns (address[] memory) 
    {
        ...
        for (uint256 i; i < currentSeasonListings.length; i ++) {
            if (curationSeasonDashboard[curationSeason][currentSeasonListings[i]] == _curator) {
    ...
    function viewArchivalSeasonSummaryByCurator(uint256 _curationSeason, address _curator )
        external
        view 
        returns (address[] memory) 
    {
        ...
        for (uint256 i; i < archivalListings.length; i ++) {
            if (viewCuratorByListingBySeason(_curationSeason, archivalListings[i]) == _curator) {
